...
- Fix #605 - Display UI notification after btc core connection test #912 (Patrick)
- Add target="_blank" for help links #911 (benk10)
- Feature: Get whitepaper via timechain #905 (Manolis)
- bugfix: Fix key purpose labeling being overwritten #887 (benk10)
- improve hints for not working connections #888 (Kim Neunert)
- Auth improvements #860 (benk10)
- add multisig tradeoffs note and doc #885 (djpnewton)
- devops: Fixed deprecation warnings #894 (Patrick)
- bugfix:  #829 - font size and horizontal alignment #900 (Patrick)
- ugly fix utxo blockexplorer rescan #897 (Stepan Snigirev)
- UIUX Fixes - TX table toolbar alignment, Network label alignment #899 (Patrick)
- Removing high level consulting request #901 (Callum Macdonald)
- Add more info to connect Desktop via TOR #847 (Ramon Tayag)
- Fix typo (puropse -> purpose) #898 (Stacie)
- Update connect-your-node.md #893 (fatrattombala)
- Make `Add Keys` more obvious #884 (Franck Royer)
- Added PyCharm IDE configuration + fixed DEVELOPMENT.md title level hierachy (incl. TOC) #892 (paeet)
- Fix Typos #879 (Franck Royer)
- feature: address-tab pagination, sorting and exporting to CSV #873 (jleo84)
- add notes about built in authentication methods #871 (djpnewton)
- Specify that hwi is not compatible with Python 3.9 #883 (Franck Royer)
- Fix tx info showing wrong input address #865 (benk10)
- Fix no block height with Bitcoin Core v0.19 #859 (benk10)
- [FIX] #784 URL encode to prevent breaking characters on RPC connection #866 (Maxi Dev)
- add rate limiting and registration link expiry #852 (djpnewton)
- [ADD] SLIP-132 switch for PDF backup key format #849 (Maxi Dev)
- Add addresses tab allowing users to navigate through derived addresses #781 (jleo84)
- Keep slashes and parentheses in tx labels #861 (Ondrej Calda)
- Add existing file #846 (bitballin)
- fixed typos in wallet_receive.jinja and wallet_settings.jinja #853 (Zach Zager)
- Added persistent tor setting. #848 (Raj)
- Check whether tx address contains list before enumerating it #855 (Ondrej Calda)
