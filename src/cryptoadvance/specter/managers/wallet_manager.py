    ...
    def update(
        ...
        if (
            ...
            if self.allow_threading and use_threading:
                ...
                def internal_update(wallets_update_list: Dict):
                    """this function is a proxy-function to copy the current_request_context before the
                    self._update method will continue the heavy lifting
                    """
                    try:
                        self.wallet_manager._update(wallets_update_list)
                    except Exception as e:
                        logger.exception(e)
                        self.is_loading = False

                t = FlaskThread(
                    target=internal_update,
