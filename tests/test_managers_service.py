...
def test_ServiceManager_get_service_packages(caplog):
    ...
    # fmt: off
    # 3 migration classes
    assert len([package for package in packages if package.startswith("cryptoadvance.specter.util.migrations.")])   == 3

    # service, controller, config, device
    assert len([package for package in packages if package.startswith("cryptoadvance.specterext.devhelp.")])        == 4

    # service, controller, device
    assert len([package for package in packages if package.startswith("cryptoadvance.specterext.electrum.")])       == 3

    # service, controller, config
    assert len([package for package in packages if package.startswith("cryptoadvance.specterext.exfund.")])         == 3
    ...
    # service, controller, config
    assert len([package for package in packages if package.startswith("cryptoadvance.specterext.notifications.")])  == 3

    # service, controller, config
    assert len([package for package in packages if package.startswith("cryptoadvance.specterext.spectrum.")])       == 3

    # service, controller, config
    assert len([package for package in packages if package.startswith("cryptoadvance.specterext.stacktrack.")])     == 3

    # service, controller, config
    assert len([package for package in packages if package.startswith("cryptoadvance.specterext.swan.")])           == 3
    # fmt: on
