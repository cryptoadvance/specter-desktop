import os
from cryptoadvance.specter.persistence import write_devices, write_device
...
def test_write_devices(app, monkeypatch, caplog):
    ...
    # count files
    def count_files_in_device_folder():
        path = app.specter.device_manager.data_folder
        return len([f for f in os.listdir(path) if os.path.isfile(os.path.join(path, f))])
    assert count_files_in_device_folder() == 2
    write_devices(devices_json)
    assert count_files_in_device_folder() == 5
...
def test_write_device(app):
    ...
    write_device(app.specter.device_manager.get_by_alias("some_name"), "/tmp/delete_me_test_file.json")
