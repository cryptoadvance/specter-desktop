...
import threading
import sys
from cryptoadvance.specter.specter_error import BrokenCoreConnectionException, SpecterError
...
def should_intercept(call):
    ''' Should return a boolean whether the visibility output should be done. 
        This probably need to be more and more restricted overtime as we hopefully
        have less and less flaky tests in the future and the normal output is enough.    
    '''
    return not (
        isinstance(call.excinfo.value, RpcError)
        or 
        isinstance(call.excinfo.value, SpecterError)
    )
...
def pytest_exception_interact(node, call, report):
    ...
    if should_intercept(call):
        ...
        print(f"intercepting: {call.excinfo.value.__class__.__name__} while {report.when}")
...
def print_debug_logs(node, call, report):
    ...
    if isinstance(call.excinfo.value, BrokenCoreConnectionException):
        ...
        for regtest in regtests:
            print("---------------------------------------------------------")
            print(f"{regtest} in DATADIR {regtest.datadir} on PORT {regtest.rpcconn.rpcport}")
