...
def create_specter_with_node_and_user(
    bitcoin_regtest, devices_filled_data_folder, node, allow_threading_for_testing
):
    ...
    config = {
        ...
        "testing": {
            "allow_threading_for_testing": allow_threading_for_testing,
        },
    }
    specter = Specter(
        data_folder=devices_filled_data_folder, config=config, checker_threads=False
    )
    node = specter.node_manager.add_external_node(
        "BTC",
        "Node with a different port",
        False,
        "",
...
def specter_regtest_configured(bitcoin_regtest, devices_filled_data_folder, node):
    specter = create_specter_with_node_and_user(
        bitcoin_regtest, devices_filled_data_folder, node, False
    )
    try:
        yield specter
    finally:
